...
import com.sksamuel.elastic4s.http.search.{SearchImplicits, SearchResponse, SearchScrollImplicits}
...
import com.sksamuel.elastic4s.searches.{ClearScrollDefinition, HighlightFieldDefinition, SearchDefinition, SearchScrollDefinition}
import com.sksamuel.elastic4s.searches.aggs.AvgAggregationDefinition
import com.sksamuel.elastic4s.searches.queries.{FuzzyQueryDefinition, InnerHitDefinition, QueryDefinition}
    ...
    with ElasticImplicits
{
  ...
  class RegisterExpectsInto(query: QueryDefinition) {
    def into(indexType: IndexAndType, field: String = "query"): IndexDefinition = {
      val src = s""" { "$field" : ${QueryBuilderFn(query).string()} } """

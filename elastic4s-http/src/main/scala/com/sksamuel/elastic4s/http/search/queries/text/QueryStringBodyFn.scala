...
object QueryStringBodyFn {
  def apply(s: QueryStringQueryDefinition): XContentBuilder = {
    ...
    s.analyzeWildcard.foreach(builder.field("analyze_wildcard", _))
    s.lenient.foreach(builder.field("lenient", _))
    s.minimumShouldMatch.foreach(builder.field("minimum_should_match", _))
    s.fuzzyMaxExpansions.foreach(builder.field("fuzzy_max_expansions", _))
    s.fuzzyPrefixLength.foreach(builder.field("fuzzy_prefix_length", _))
    s.fuzziness.foreach(builder.field("fuzziness", _))
    s.phraseSlop.foreach(builder.field("phrase_slop", _))
    s.autoGeneratePhraseQueries.foreach(builder.field("auto_generate_phrase_queries", _))
    s.allowLeadingWildcard.foreach(builder.field("allow_leading_wildcard", _))
    s.enablePositionIncrements.foreach(builder.field("enable_position_increments", _))
    s.boost.foreach(builder.field("boost", _))
    s.quoteFieldSuffix.foreach(builder.field("quote_field_suffix", _))
    s.splitOnWhitespace.foreach(builder.field("split_on_whitespace", _))
